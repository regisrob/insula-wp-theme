...
basics_scripts()				Load other Javascripts in footer	//#!edited
 ...
 * Problem introduced with WP 3.4 : captions broken for new posts (created after WP 3.4 core update) : original Basics function needed to be desactivated because of an issue with add_shortcode( 'caption', 'some_function_in_their_theme' );
 */
/*
...
/* Fix with the help of add_filter : http://wordpress.org/support/topic/caption-broken-in-new-34-posts (fixed by andidas) */
...
function basics_jquery() {
	$path_to_jquery = get_template_directory_uri() . '/js/libs/jquery-1.7.2.min.js';
...
<script defer src="<?php echo get_stylesheet_directory_uri(); ?>/js/fancybox/jquery.fancybox-1.3.4.pack.js"></script>
...
?>
<section>
	<h1 class="section-title hidden"><?php _e( 'Search in', 'basics' ); ?></h1>
	<form action="<?php echo home_url( '/' ); ?>" method="get" role="search">
		<fieldset>
			<label for="search" class="visuallyhidden focusable"><?php _e( 'Search in ','basics' ); ?> <?php bloginfo( 'name' ); ?></label>
			...
			<input type="submit" value="<?php _e( 'Search in','basics' ); ?>" class="visuallyhidden" />
		</fieldset>
	</form>
</section>
<?php  
